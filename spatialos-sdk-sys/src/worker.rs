...
/// An error callback, used to signal error messages for debugging.
...
/// Parameters to configure flow control. Used by Worker_ModularUdpNetworkParameters.
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
/// Parameters for configuring KCP for a modular UDP connection. Used by
...
/// Parameters for configuring a modular UDP connection. Used by Worker_NetworkParameters.
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
/// Use this flag to connect over a modular UDP stack.
...
pub struct Worker_NetworkParameters {
    ...
    /// Parameters used if the WORKER_NETWORK_MODULAR_UDP flag is set.
...
extern "C" {
    ...
    ///
    /// Note: We do not recommend reserving entity IDs. Instead, we recommend using Worker_SendCreateEntityRequest
    /// without specifying an entity ID. You can then use the automatically assigned entity ID provided in
...
extern "C" {
    ...
    /// component_update->schema_type is set to NULL. If component_udpate->user_handle is set, then it
...
/// Struct to keep track of the state of SnapshotInputStreams and SnapshotOutputStreams.
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
extern "C" {
    /// Writes next entity_id, entity pair from input. Must call Worker_SnapshotOutputStream_GetState and
    /// Worker_SnapshotOutputStream_GetError to get any error that occurred during operation.
