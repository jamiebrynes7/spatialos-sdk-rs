...
pub mod improbable {
    ...
    impl TypeConversion for ComponentInterest_Query {
        ...
        fn to_type(input: &Self, output: &mut SchemaObject) -> Result<(), String> {
            <generated::improbable::ComponentInterest_QueryConstraint as TypeConversion>::to_type(
                &&input.constraint,
                &mut output.field::<SchemaObject>(1).add(),
            )?;
    ...
    impl TypeConversion for ComponentInterest_QueryConstraint {
        fn from_type(input: &SchemaObject) -> Result<Self, String> {
            Ok(Self {
                ...
                relative_sphere_constraint: if let Some(data) = input.field::<SchemaObject>(4).get()
                {
                ...
                relative_box_constraint: if let Some(data) = input.field::<SchemaObject>(6).get() {
