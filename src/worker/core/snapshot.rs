...
impl SnapshotOutputStream {
    pub fn new<P: AsRef<Path>>(filename: P, params: SnapshotParameters) -> Self {
        ...
        let ptr = unsafe { Worker_SnapshotOutputStream_Create(filename_cstr.as_ptr(), &params.to_worker_sdk()) };
    ...
    pub fn write_entity(&self, snapshot: &EntitySnapshot) -> Result<(), String> {
        let _ = unsafe { Worker_SnapshotOutputStream_WriteEntity(self.internal_ptr, &snapshot.to_worker_sdk()) };
