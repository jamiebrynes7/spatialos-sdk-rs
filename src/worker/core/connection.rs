...
trait Connection {
    fn send_log_message(level: LogLevel, logger_name: &str, message: &str, entity_id: Option<EntityId>);
    ...
    fn send_command_request(entity_id: EntityId, request: CommandRequest, command_parameters: CommandParameters) -> RequestId<OutgoingCommandRequest>;
    fn send_command_response(request_id: RequestId<IncomingCommandRequest>, response: CommandResponse);
...
impl WorkerConnection {
    pub fn connect_receptionist_async(worker_id: &str, params: ReceptionistConnectionParameters) -> WorkerConnectionFuture {
        let hostname_cstr = CString::new(params.hostname).expect("Received 0 byte in supplied hostname.");
        let worker_id_cstr = CString::new(worker_id).expect("Received 0 byte in supplied Worker ID");
        
        let future_ptr = unsafe { Worker_ConnectAsync(hostname_cstr.as_ptr(), params.port, worker_id_cstr.as_ptr(), &params.connection_params.to_worker_sdk()) };
...
impl WorkerConnectionFuture {
    ...
    pub fn get(&self, timeout_millis: u32) -> Option<WorkerConnection> {
        ...
        let connection_ptr = unsafe { Worker_ConnectionFuture_Get(self.future_ptr, &timeout_millis) };
